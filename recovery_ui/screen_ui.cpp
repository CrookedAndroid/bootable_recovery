...
void ScreenRecoveryUI::SetColor(UIElement e) const {
  switch (e) {
    case UIElement::INFO: // Recovery info #F0E68C
      if (fastbootd_logo_enabled_)
        gr_color(0xF0, 0xE6, 0x8C, 255);
      else
        gr_color(0xF0, 0xE6, 0x8C, 255);
      break;
    case UIElement::HEADER: // Category header #00C853
      if (fastbootd_logo_enabled_)
        gr_color(0x0, 0xC8, 0x53, 255);
      else
        gr_color(0x0, 0xC8, 0x53, 255);
      break;
    case UIElement::MENU: // menu text color #C0C0C0
      gr_color(0xC0, 0xC0, 0xC0, 255);
      break;
    case UIElement::MENU_SEL_BG:
    case UIElement::SCROLLBAR: // Menu bar color #1A2228
      if (fastbootd_logo_enabled_)
        gr_color(0x1A, 0x22, 0x28, 255);
      else
        gr_color(0x1A, 0x22, 0x28, 255);
      break;
    case UIElement::MENU_SEL_BG_ACTIVE:
      ...
      break;
    case UIElement::MENU_SEL_FG: // Menu bar text #778899
      if (fastbootd_logo_enabled_)
...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : crooked_logo;
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  crooked_logo = LoadBitmap("logo_image");
...
int ScreenRecoveryUI::SelectMenu(const Point& p) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(crooked_logo.get());
      const static int logo_height = gr_get_height(crooked_logo.get());
